...
-- Add search history to user preferences calculation
-- =====================================================

-- Drop the old function
DROP FUNCTION IF EXISTS calculate_user_preferences(UUID);

-- Create improved function that includes search history
...
BEGIN
  ...
  SELECT
    ...
    -- Interactions get weight 1.0, searches get weight 0.5
    (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          'location', location,
          'weight', ROUND((weighted_count::numeric / GREATEST(total_interactions + (total_searches * 0.5), 1)), 2)
...
COMMENT ON FUNCTION calculate_user_preferences IS 'Analyzes user interactions AND search history to update their preference profile';
