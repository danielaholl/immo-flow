...
import { createProperty, sendChatMessage } from '@immoflow/api';
import { PropertyImagePlaceholder } from '@immoflow/ui';
import { Building2, Home, Castle, Warehouse, Sparkles, Check, ChevronRight, ImagePlus, X, Send } from 'lucide-react';
...
export default function CreateListingPage() {
  ...
  // Get all images (uploaded or saved) - use the same variable calculated before early returns
  const previewImages = currentPreviewImages;
      ...
      <div className="relative h-72 group">
                ...
                >
                  <div
                    className={`h-full rounded-full transition-all duration-300 ${
                      idx < previewImageIndex ? 'bg-white w-full' :
        ...
        {listingData.property_type && (
          <div className="absolute top-8 left-3 z-10">
            <span className="bg-white shadow-md px-3 py-1 rounded-full text-sm font-semibold text-gray-800">
              {PROPERTY_TYPE_LABELS[listingData.property_type]}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-between mb-2">
            <p className="text-base text-gray-500 font-medium">
              {listingData.location || <span className="text-gray-300">Standort</span>}
            </p>
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mb-3 leading-tight">
            {listingData.title || <span className="text-gray-300">Titel Ihrer Immobilie</span>}
          </h1>

          {/* Key Details Row - Airbnb style */}
          <div className="flex items-center gap-1.5 text-base text-gray-600 mb-4 flex-wrap">
            {listingData.sqm && (
              <>
                <span>{listingData.sqm} m²</span>
                <span className="text-gray-300">·</span>
              </>
            )}
            {listingData.rooms && (
              <>
                <span>{listingData.rooms} Zimmer</span>
                <span className="text-gray-300">·</span>
              </>
            )}
            {listingData.condition && (
              <span>{CONDITION_LABELS[listingData.condition]}</span>
            )}
            {!listingData.sqm && !listingData.rooms && !listingData.condition && (
              <span className="text-gray-300">Details werden hier angezeigt</span>
            )}
          </div>
          ...
          {/* Features - Airbnb style grid */}
          {listingData.features && listingData.features.length > 0 ? (
            <div className="mb-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Was diese Unterkunft bietet</h3>
            ...
            <div className="mb-4 py-6 border border-dashed border-gray-200 rounded-xl">
              <p className="text-base text-gray-400 text-center">Ausstattungsmerkmale</p>
            </div>
          )}
          ...
          {listingData.description ? (
            ...
            <div className="py-4 border border-dashed border-gray-200 rounded-xl mb-4">
              <p className="text-base text-gray-400 text-center">Beschreibung</p>
            </div>
          )}
          ...
          {listingData.viewing_appointments && listingData.viewing_appointments.length > 0 ? (
            ...
            <div className="py-4 border border-dashed border-gray-200 rounded-xl mb-4">
              <p className="text-base text-gray-400 text-center">Besichtigungstermine</p>
            </div>
          )}
              ...
              <div className="flex items-center gap-3">
                {profile?.avatar_url ? (
                  ...
                  <div className="w-12 h-12 bg-primary rounded-full flex items-center justify-center text-white font-bold text-lg">
                    {profile?.first_name?.charAt(0)?.toUpperCase() || user?.email?.charAt(0).toUpperCase() || 'M'}
                  </div>
                )}
                <div className="flex-1">
                  ...
                  {profile?.company && (
                    <div className="text-sm text-primary font-medium">{profile.company}</div>
                  )}
      ...
      <div className="border-t border-gray-200 p-4 bg-white flex-shrink-0">
        {isComplete ? (
          <div className="space-y-2">
            ...
            >
              {isSubmitting ? 'Veröffentliche...' : 'Inserat veröffentlichen'}
            </button>
            <button
              onClick={() => {
                setMessages([]);
                ...
                setPreviewImageIndex(0);
                setTextInput('');
                setTimeout(() => addBotMessage(0), 100);
              }}
              className="w-full py-2 text-gray-600 font-medium hover:bg-gray-50 rounded-lg transition-colors text-base"
            >
              Neu starten
            </button>
          </div>
            ...
            <div>
              ...
              {previewPricePerSqm > 0 && (
                <span className="text-sm text-gray-500 ml-2">
                  ({formatPrice(previewPricePerSqm)}/m²)
                </span>
              )}
            </div>
            <button
              ...
              className="bg-primary text-white px-6 py-3 rounded-lg font-semibold text-base hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {isSubmitting ? 'Veröffentliche...' : 'Veröffentlichen'}
            </button>
          </div>
        )}
      </div>
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="h-[calc(100vh-64px)] flex flex-col px-4 py-4">
        ...
        {/* Two Column Layout - Responsive: Preview bottom on mobile, side-by-side on desktop */}
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 max-w-6xl mx-auto w-full flex-1 min-h-0 overflow-auto lg:overflow-hidden">
                ...
                {isComplete && (
                  <div className="flex gap-3 justify-start">
                    ...
                    <p className="text-[18px] leading-relaxed text-green-700 pt-2 max-w-[85%]">
                      Perfekt! 🎉 Ihr Inserat ist fertig. Überprüfen Sie die Vorschau rechts und klicken Sie auf &quot;Veröffentlichen&quot;.
                    </p>
                  </div>
                )}
          ...
          {/* Preview Column - Shows after chat on mobile, right side on desktop */}
