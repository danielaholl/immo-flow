...
ALTER TABLE properties ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES public.users(id);

-- 2. Create index for faster lookups
CREATE INDEX IF NOT EXISTS idx_properties_user_id ON properties(user_id);

-- 3. Copy existing seller_id values to user_id (skip if seller_id doesn't exist)
...
ALTER TABLE bookings ADD COLUMN IF NOT EXISTS owner_id UUID REFERENCES public.users(id);
